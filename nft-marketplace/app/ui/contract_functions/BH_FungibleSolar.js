...
async function balanceOfFS(loggedInAddress) {
  ...
  const contract = new ethers.Contract(BH_FungibleSolar_address, BH_FungibleSolar_abi.abi, signer);
...
async function stake_HouseT(loggedInAddress, tokenID = "", setApproveStaking = ()=>{}) {
    ...
    if(approved_address != BH_FungibleSolar_address){
...
async function approve_USDC(loggedInAddress, amount = ""){
  ...
  console.log(contract)
  console.log(amountEther)
  const transaction = await contract.approve(addresses.BH_FungibleSolar_address, amountEther);
